...
<div class="flex flex-row flex-wrap md:mt-12">
	<div class="mt-4 flex w-full flex-col md:mt-0 lg:w-2/3 lg:pr-10">
		<h4 class="text-md w-full border-b pb-2 font-light">Self Published Posts</h4>
		<div class="relative mt-6 columns-1 space-y-6">
			{#each data.allPosts as item, _}
				<a href={`/blog/${item.slug}`} class="flex flex-col border-gray-200">
					<div class="flex break-inside-avoid flex-col">
						...
						<span class="mt-3 line-clamp-2 text-sm font-light text-slate-600">{item.content.intro}</span>
	...
	<div class="mt-8 flex w-full flex-col md:mt-0 lg:w-1/3">
		<h4 class="text-md w-full border-b pb-2 font-light">Media Posts</h4>
		{#each data.recommendedPosts as item}
			<a rel="noopener" target="_blank" href={item.content.Url.url} class="mt-6 truncate border-b pb-2 text-sm font-light text-slate-600 hover:underline">
				{item.content.Title}
			</a>
		{/each}
	</div>
